import html from './assistant-seat.html';
...
export default {
	...
	computed: {
		...mapState({
			...
			agentAssistantList: state => state.AgentAssistantController.list,
	...
	data() {
		return {
			...
			addSeatDataRules: {
				seatId: [{
					required: true, message: '请选择小职姐助理', trigger: 'blur'
				}],
				extNo: [{
	...
	methods: {
		...mapActions(['CALL_SEAT', 'ANGENT_ASSISTANT_LIST', 'CALL_SIP_PHONE', 'CALL_ADD', 'CALL_DELETE', 'CALL_UNBIND', 'CALL_BIND_EDIT']),
		getList() {
			this.loading = true;
		...
		getAgentAssistantList() {
            this.ANGENT_ASSISTANT_LIST({headers: {Authorization: 'Basic d2ViLWFwcDo='}}).then(res => {});
		},
		...
		deletSeat(seatId, nickname) {
			this.$confirm('确定删除坐席' + nickname + '?', '提示', {
				...
				this.CALL_DELETE({body: {seatId: seatId, seatType: 'CUSTOMER'}, headers: {Authorization: 'Basic d2ViLWFwcDo='}}).then(res => {
					if (res === '0') {
						this.$message({
							message: '删除坐席成功',
							type: 'success'
		...
		unbindSeat(seatId, nickname) {
			this.$confirm('确定解绑坐席' + nickname + '?', '提示', {
				...
				this.CALL_UNBIND({body: {seatId: seatId, seatType: 'CUSTOMER'}, headers: {Authorization: 'Basic d2ViLWFwcDo='}}).then(res => {
					if (res === '0') {
						this.$message({
							message: '解绑坐席成功',
							type: 'success'
